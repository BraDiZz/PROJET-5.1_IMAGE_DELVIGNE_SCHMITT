...
void click_scheme(Gtk::Window& parent_window,Gtk::Image& ImageOut,char image_path[250],Gtk::SpinButton& buttonHue1,Gtk::SpinButton& buttonDistance,Gtk::SpinButton &buttonNumberHues,int& Scheme_mode) {
    if(buttonHue1.get_value()==-1)buttonHue1.set_value(360);
    if(buttonHue1.get_value()==361)buttonHue1.set_value(0);
    ColorImage img(image_path);
    if(Scheme_mode==1){
        ClosestMapper mapper(new MonochromeColorScheme(buttonHue1.get_value()));
        mapper.ConvertToColorScheme(img);
    }
    if(Scheme_mode==2){
        ClosestMapper mapper(new ComplementaryColorScheme(buttonHue1.get_value()));
        mapper.ConvertToColorScheme(img);
    }
    if(Scheme_mode==3){
        ClosestMapper mapper(new TriadicColorScheme(buttonHue1.get_value()));
        mapper.ConvertToColorScheme(img);
    }
    if(Scheme_mode==4){
        ClosestMapper mapper(new AnalogousColorScheme(buttonHue1.get_value(),buttonDistance.get_value(),buttonNumberHues.get_value()));
        mapper.ConvertToColorScheme(img);
    }
...
void click_mono(Gtk::Window& parent_window,int &Scheme_mode,Gtk::Grid &grid,Gtk::SpinButton &buttonHue1) {
    Scheme_mode=1; 
...
void click_comp(Gtk::Window& parent_window,int &Scheme_mode,Gtk::Grid &grid,Gtk::SpinButton &buttonHue1) {
    Scheme_mode=2; 
    Gtk::Widget* widget_to_keep = grid.get_child_at(0, 3);
    if (!widget_to_keep) {
        grid.attach(buttonHue1,0,3,1,1);
...
void click_triadic(Gtk::Window& parent_window,int &Scheme_mode,Gtk::Grid &grid,Gtk::SpinButton &buttonHue1) {
    Scheme_mode=3;
    Gtk::Widget* widget_to_keep = grid.get_child_at(0, 3);
    if (!widget_to_keep) {
        grid.attach(buttonHue1,0,3,1,1);
...
void click_Analogus(Gtk::Window& parent_window,int &Scheme_mode,Gtk::Grid &grid,Gtk::SpinButton &buttonHue1,Gtk::SpinButton &buttonDistance,Gtk::SpinButton &buttonNumberHues) {
    Scheme_mode=4;
    Gtk::Widget* widget_to_keep = grid.get_child_at(0, 3);
...
int main(int argc, char* argv[]){
    ...
    buttonHue1.signal_value_changed().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() {
        click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);
    });
    buttonDistance.signal_value_changed().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() {
        click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);
    });
    buttonNumberHues.signal_value_changed().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() {
        click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);
    });
    ...
    buttonMono.signal_clicked().connect([&window,&Scheme_mode,&grid,&buttonHue1]() { click_mono(window,Scheme_mode,grid,buttonHue1); });
    buttonMono.signal_clicked().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() { click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);});

    Gtk::Button buttonComp("Complémentaire");
    buttonComp.signal_clicked().connect([&window,&Scheme_mode,&grid,&buttonHue1]() { click_comp(window,Scheme_mode,grid,buttonHue1); });
    buttonComp.signal_clicked().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() { click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);});

    Gtk::Button buttonTria("Triadic");
    buttonTria.signal_clicked().connect([&window,&Scheme_mode,&grid,&buttonHue1]() { click_triadic(window,Scheme_mode,grid,buttonHue1); });
    buttonTria.signal_clicked().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() { click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);});

    Gtk::Button buttonAna("Analogue");
    buttonAna.signal_clicked().connect([&window,&Scheme_mode,&grid,&buttonHue1,&buttonDistance,&buttonNumberHues]() { click_Analogus(window,Scheme_mode,grid,buttonHue1,buttonDistance,buttonNumberHues); });
    buttonAna.signal_clicked().connect([&window, &ImageOut, &image_path, &buttonHue1,&buttonDistance,&buttonNumberHues, &Scheme_mode]() { click_scheme(window, ImageOut, image_path, buttonHue1,buttonDistance,buttonNumberHues,Scheme_mode);});
